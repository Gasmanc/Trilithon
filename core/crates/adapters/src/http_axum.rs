...
pub struct AppState {
    ...
    /// Drift detector — exposes resolution operations to HTTP handlers.
...
#[utoipa::path(
    ...
    path = "/api/v1/health",
    responses(
        (status = 200, description = "Daemon is ready"),
        (status = 503, description = "Daemon is still starting"),
    )
)]
...
/// Full `OpenAPI` 3.1 document generated by `utoipa`.
///
/// Each handler in slices 9.1–9.11 is listed here.  The `#[utoipa::path]`
/// attributes on those handlers supply the per-route metadata.
#[derive(OpenApi)]
#[openapi(
    paths(
        health_handler,
        ...
        mutations::post_mutation,
        snapshots::list_snapshots,
        snapshots::get_snapshot,
        snapshots::diff_snapshots,
        routes::list_routes,
        audit_routes::list_audit,
        drift_routes::current,
        drift_routes::adopt,
        drift_routes::reapply,
        drift_routes::defer,
        capabilities::get_capabilities,
    ),
    info(
        title = "Trilithon Daemon API",
        version = env!("CARGO_PKG_VERSION"),
        description = "HTTP API for the Trilithon reverse-proxy management daemon.",
    )
)]
pub struct ApiDoc;
...
///
/// Returns the full `OpenAPI` 3.1 document as pretty-printed JSON.
async fn openapi_handler() -> Json<Value> {
    let doc = ApiDoc::openapi();
    let json_str = doc.to_pretty_json().unwrap_or_else(|_| "{}".to_owned());
...
pub fn router(state: Arc<AppState>) -> Router {
    Router::new()
        ...
        .route("/api/v1/drift/current", get(drift_routes::current))
        .route("/api/v1/drift/{event_id}/adopt", post(drift_routes::adopt))
        .route(
            "/api/v1/drift/{event_id}/reapply",
            post(drift_routes::reapply),
        )
        .route("/api/v1/drift/{event_id}/defer", post(drift_routes::defer))
