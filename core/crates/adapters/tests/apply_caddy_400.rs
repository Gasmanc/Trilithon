//! Slice 7.4 — Caddy 400 rejection test.
//!
//! A fake `CaddyClient` that returns `BadStatus { status: 400 }`.  Asserts
//! that:
//! - `apply` returns `Ok(ApplyOutcome::Failed { kind: CaddyValidation, .. })`.
//! - Exactly one `config.apply-failed` audit row is written.
//! - No `config.applied` row exists.
...
use trilithon_core::{
    ...
    reconciler::{Applier, ApplyFailureKind, ApplyOutcome, DefaultCaddyJsonRenderer},
    schema::SchemaRegistry,
    storage::{
        trait_def::Storage,
        types::{AuditOutcome, AuditSelector, Snapshot, SnapshotId},
    },
};
...
async fn caddy_400_returns_failed_caddy_validation() {
    ...
    match outcome {
        ...
        other => panic!("expected Failed{{CaddyValidation}}, got {other:?}"),
    }
}
...
async fn caddy_400_writes_exactly_one_apply_failed_row() {
    ...
    assert_eq!(rows[0].error_kind.as_deref(), Some("CaddyValidation"));
}
...
async fn caddy_400_no_config_applied_row() {
    ...
    assert!(
        rows.is_empty(),
        "no config.applied row must exist on 400 failure"
    );
}
