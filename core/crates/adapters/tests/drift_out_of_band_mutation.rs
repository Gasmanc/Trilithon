//! Slice 8.5 — detect drift when Caddy config diverges from desired state.
...
/// Returns a `DesiredState` with an extra field to simulate drift.
...
#[tokio::test]
async fn drift_out_of_band_mutation() {
    ...
    assert!(
        matches!(outcome, TickOutcome::Drifted { .. }),
        "expected Drifted, got {outcome:?}"
    );
}
