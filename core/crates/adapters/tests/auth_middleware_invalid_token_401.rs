//! Bogus bearer token → middleware returns 401 with `{"code":"unauthenticated"}`.
...
#[tokio::test]
async fn auth_middleware_invalid_token_401() {
    let (_dir, addr, tx) = setup().await;

    let client = reqwest::Client::new();
    let resp = client
        .post(format!("http://{addr}/api/v1/auth/logout"))
        .header("Authorization", "Bearer bogus-token-does-not-exist")
        .send()
        .await
        .expect("request");
    ...
    assert_eq!(
        ...
        "body must carry unauthenticated code"
    );
