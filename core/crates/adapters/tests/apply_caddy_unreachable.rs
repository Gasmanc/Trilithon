//! Slice 7.4 — Caddy unreachable test.
//!
//! A fake `CaddyClient` that returns `CaddyError::Unreachable { .. }` on
//! `load_config`.  Asserts that:
//! - `apply` returns `Err(ApplyError::Unreachable { .. })`.
//! - Exactly one `caddy.unreachable` audit row is written.
...
async fn caddy_unreachable_returns_err() {
    ...
    assert!(
        matches!(err, ApplyError::Unreachable { .. }),
        "expected ApplyError::Unreachable, got {err:?}"
    );
}
...
async fn caddy_unreachable_writes_caddy_unreachable_audit_row() {
    ...
    assert_eq!(rows.len(), 1, "exactly one caddy.unreachable row");
    assert_eq!(rows[0].kind, "caddy.unreachable");
}
