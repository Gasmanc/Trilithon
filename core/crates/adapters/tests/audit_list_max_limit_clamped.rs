//! `GET /api/v1/audit?limit=5000` — clamped to at most 1000 rows.
...
async fn setup() -> (
    ...
    user_store
        .create_user("bob", "correct-horse-battery-staple", UserRole::Owner)
        .await
        .expect("create user");
    ...
    // Insert 1100 rows so we can verify the 1000 cap.
    for i in 0u32..1100 {
...
#[tokio::test]
async fn audit_list_max_limit_clamped() {
    let (_dir, addr, tx, session_id) = setup().await;

    let client = reqwest::Client::new();
    let resp = client
        .get(format!("http://{addr}/api/v1/audit?limit=5000"))
    ...
    assert!(
        rows.len() <= 1000,
        "limit must be clamped to 1000 but got {}",
        rows.len()
    );
    assert_eq!(
        rows.len(),
        1000,
        "1100 rows in store with limit=5000 must return exactly 1000"
    );
