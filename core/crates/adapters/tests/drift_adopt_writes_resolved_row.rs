...
/// Insert a minimal snapshot and a drift event pointing at it.
async fn seed_drift(storage: &Arc<dyn Storage>) -> DriftRowId {
    ...
    let snap = Snapshot {
        ...
        correlation_id: "01ARZ3NDEKTSV4RRFFQ69G5FAD".to_owned(),
    ...
    let drift_row = DriftEventRow {
        id: row_id.clone(),
        correlation_id: "01ARZ3NDEKTSV4RRFFQ69G5FAA".to_owned(),
...
#[tokio::test]
async fn drift_adopt_writes_resolved_row() {
    ...
    let client = reqwest::Client::new();
    let resp = client
        .post(format!("http://{addr}/api/v1/drift/{}/adopt", row_id.0))
