//! POST to a protected endpoint without any auth credentials → 401.
...
#[tokio::test]
async fn auth_middleware_unauthenticated_mutation_401() {
    let (_dir, addr, tx) = setup().await;

    // POST to logout (a protected mutation endpoint) without any credentials.
    ...
    assert_eq!(resp.status(), 401, "unauthed mutation must return 401");
