//! Integration test: `DELETE FROM audit_log` is aborted by the immutability trigger.
...
/// Attempting `DELETE FROM audit_log WHERE id = ?` must be aborted by the
/// `audit_log_no_delete` trigger and the error must propagate.
#[tokio::test]
async fn delete_audit_log_aborts() {
    let pool = test_pool().await;
    apply_migrations(&pool)
        .await
        .expect("migrations must succeed");

    let id = "01JTEST000000000000000DELETE";
    ...
    assert!(
        result.is_err(),
        "DELETE FROM audit_log must fail; immutability trigger should abort"
    );
