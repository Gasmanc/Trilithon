...
mod apply_audit_notes_format_seam {
    ...
    fn notes_to_string_produces_valid_json() {
        ...
        assert!(parsed.is_ok(), "notes_to_string must produce valid JSON; got: {s}");
    }
    ...
    fn notes_to_string_keys_are_sorted() {
        let notes = ApplyAuditNotes {
            reload_kind: ReloadKind::Graceful { drain_window_ms: Some(200) },
            applied_state: AppliedStateTag::Applied,
        ...
        assert_eq!(keys, sorted, "notes_to_string keys must be lexicographically sorted");
    }
}
