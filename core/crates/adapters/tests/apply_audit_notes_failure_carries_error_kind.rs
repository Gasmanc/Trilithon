//! Slice 7.7 — failure audit notes carry `error_kind`.
//!
//! A Caddy 400 response must produce a `config.apply-failed` row whose `notes`
//! parses to [`ApplyAuditNotes`] with `error_kind = Some("CaddyValidation")`.
...
use trilithon_core::{
    ...
    reconciler::{Applier, ApplyAuditNotes, DefaultCaddyJsonRenderer},
...
impl CaddyClient for BadStatusClient {
    async fn load_config(&self, _: CaddyConfig) -> Result<(), CaddyError> {
        Err(CaddyError::BadStatus {
            status: 400,
            body: "invalid config field: xyz".to_owned(),
        })
    }
...
fn build_applier(storage: Arc<dyn Storage>, lock_pool: SqlitePool) -> CaddyApplier {
    ...
    CaddyApplier {
        client: Arc::new(BadStatusClient),
...
/// A Caddy 400 rejection must produce a `config.apply-failed` row whose `notes`
/// carries `error_kind = Some("CaddyValidation")` and `caddy_status = Some(400)`.
#[tokio::test]
async fn audit_notes_failure_carries_error_kind() {
    ...
    applier
        .apply(&snapshot, 0)
        .await
        .expect("apply must return Ok even on 400");
    ...
    assert_eq!(rows.len(), 1, "exactly one config.apply-failed row");

    let notes_str = rows[0]
        .notes
        .as_deref()
        .expect("notes must be Some on config.apply-failed");

    let notes: ApplyAuditNotes =
        serde_json::from_str(notes_str).expect("notes must parse as ApplyAuditNotes");

    assert_eq!(
        notes.error_kind.as_deref(),
        Some("CaddyValidation"),
        "error_kind must be CaddyValidation"
    );
    assert_eq!(notes.caddy_status, Some(400), "caddy_status must be 400");
    assert!(
        notes.error_detail.is_some(),
        "error_detail must be populated with the Caddy response body excerpt"
    );
}
