//! `GET /api/v1/audit?event=auth.login-succeeded` — only matching rows returned.
...
async fn setup() -> (
    ...
    user_store
        .create_user("carol", "correct-horse-battery-staple", UserRole::Owner)
        .await
        .expect("create user");
    ...
    // Insert 5 rows with "auth.login-succeeded" and 5 with "config.applied".
    for i in 0u32..5 {
        let row = AuditEventRow {
            ...
            kind: "auth.login-succeeded".to_owned(),
            target_kind: None,
            target_id: None,
            snapshot_id: None,
    ...
    for i in 0u32..5 {
        let row = AuditEventRow {
            ...
            caddy_instance_id: "local".to_owned(),
            correlation_id: ulid::Ulid::new().to_string(),
            occurred_at: 1_700_001_000 + i64::from(i),
            occurred_at_ms: (1_700_001_000 + i64::from(i)) * 1000,
            actor_kind: ActorKind::System,
            actor_id: "daemon".to_owned(),
...
#[tokio::test]
async fn audit_list_event_filter() {
    let (_dir, addr, tx, session_id) = setup().await;

    let client = reqwest::Client::new();
    let resp = client
        .get(format!(
            "http://{addr}/api/v1/audit?event=auth.login-succeeded"
        ))
    ...
    assert_eq!(rows.len(), 5, "event filter must return only matching rows");

    for row in rows {
        assert_eq!(
            row["event"], "auth.login-succeeded",
            "all returned rows must match the event filter"
        );
    }
