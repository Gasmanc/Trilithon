...
| `/storage/.*` ignore rule covering the full `/storage/` namespace | F026/F088: this pattern matches `/storage/trilithon-owner` (the ADR-0015 ownership sentinel), making sentinel removal completely invisible to the drift detector; narrowed to `/storage/acme/` and `/storage/ocsp/` with an explicit carve-out. F088 confirmed the broad pattern remained in the TODO and was corrected in round 10 |
...
| `Arc::unwrap_or_clone(detector)` in task spawn | F090: has consuming semantics on single-owner fast path; wrong idiom for shared ownership. Use explicit `Arc::clone` into task closure |
| New Mutation variants without Phase 7 applier match-arm updates | F089: `ReplaceDesiredState`, `ReapplySnapshot`, `DriftDeferred` added to enum but no match arms in Phase 7 applier — wildcard arm silently discards adopt/reapply mutations. Slice 8.4 must update the applier in the same commit |
...
7. `DriftDetectorConfig` must have a `validate()` method that enforces the [10, 3600] seconds interval constraint at construction time; both zero (panic) and extremely large values (silent dead detector) must be rejected at config-load time before `tokio::time::interval` is ever called. `from_settings` must return `Result` and reject out-of-range values.
...
18. The `AuditWriter` write and the `storage.record_drift_event` write inside `record()` must be treated as an atomic unit; either they share a SQLite transaction, or a unique constraint on `(kind, correlation_id)` in `audit_log` must be the backstop with explicit `UNIQUE` violation handling. Steps 3 and 4 of Slice 8.6 MUST execute within a single transaction.
...
21. Task spawn must use explicit `Arc::clone(&detector)` into the task closure, not `Arc::unwrap_or_clone`. The original `Arc` is retained for Phase 9 `AppState`.
22. Slice 8.4 must update the Phase 7 applier match arms for all three new `Mutation` variants (`ReplaceDesiredState`, `ReapplySnapshot`, `DriftDeferred`) in the same commit. A cross-phase integration test must verify each variant reaches the correct Caddy admin call (or no call for `DriftDeferred`).
...
| 10 | 0 | 2 | 2 | 1 | `/storage/.*` broad pattern remained in TODO despite constraint adoption; new Mutation variants lack Phase 7 applier match arms |
