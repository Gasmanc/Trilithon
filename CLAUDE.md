...
This project has adopted the cross-phase coherence layer (see `docs/cross-phase/PLAN.md`
or `~/.claude/skills/coherence-audit/`).

- Public API contracts live in `docs/architecture/contracts.md` (generated by
  `just registry-regen` — never hand-edit).
- Invariants for those contracts live in `docs/architecture/contracts-invariants.md`
  (human-curated; updated alongside contract changes).
- Architectural seams are enumerated in `docs/architecture/seams.md`. Cross-phase
  integration tests live under `tests/cross_phase/`.
- Findings carry the Foundation 0 schema (see `docs/cross-phase/finding-schema.md`).
  Identity is `<category>:<location>:<finding_kind>` — no prose-similarity dedup.
- `/coherence-audit` returns a verdict (WORKING / DEGRADED / BROKEN) for the system
  as a whole. Run after every `/project-audit` cycle.
